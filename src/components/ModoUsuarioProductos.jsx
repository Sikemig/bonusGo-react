...
export default function ModoUsuarioProductos() {
  ...
  const fetchProductos = async () => {
    ...
    try {
      const responseProductos = await axios.get(`/producto/getall`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const productos = responseProductos.data;

      // Verificar los productos canjeados del backend
      const responseCanjeados = await axios.get(`/transacciones/canjeados/${idUsuario}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleCanjearProducto = async (id_Producto) => {
    ...
    const idUsuario = localStorage.getItem('id');
    try {
      await axios.post(`/transacciones/canjear?idProducto=${id_Producto}&idUsuario=${idUsuario}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      const mensaje = error?.response?.data || 'Ha ocurrido un error inesperado al canjear el producto.';
      alert(mensaje);
    }
  };
